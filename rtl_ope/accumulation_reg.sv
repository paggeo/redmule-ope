...
#(
  ...
  logic [$clog2(DEPTH)-1:0]         read_index_d, read_index_q;
  logic [$clog2(DEPTH)-1:0]         write_index_d, write_index_q;
  ...
  always_comb begin
    ...
    if (in_valid_i) begin
      ...
      write_index_d                 = (DEPTH > 1) ? ((write_index_q == DEPTH-1) ? '0 : write_index_q + 1) : '0;
    end

    if (read_i) begin
      output_o      = internal_reg_q[read_index_q];
      out_valid_o   = 1'b1;
      read_index_d  = (DEPTH > 1) ? ((read_index_q == DEPTH-1) ? '0 : read_index_q + 1) : '0;
    end
  end
